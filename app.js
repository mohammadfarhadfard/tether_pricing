...
async function getOrderBook(action, amountRequested) {
    try {
        ...
        if (action.toLowerCase() === "sell") {
            maxPrice = Math.max(totalPriceNobitex / 10, totalPriceWallex, totalPriceRamzinex / 10);
            bestExchange = maxPrice === totalPriceNobitex / 10 ? "NOBITEX" : maxPrice === totalPriceWallex ? "WALLEX" : "Ramzinex";
        } else {
            //for buying, keep the previous logic
            maxPrice = Math.min(totalPriceNobitex / 10, totalPriceWallex, totalPriceRamzinex / 10);
            bestExchange = maxPrice === totalPriceNobitex / 10 ? "NOBITEX" : maxPrice === totalPriceWallex ? "WALLEX" : "Ramzinex";
        }
